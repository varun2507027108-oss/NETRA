...
object NetraVision {
    ...
    fun prepass(imageB64: String, optionsJson: String = "{}"): String {
        ...
        try {
            ...
            work = if (workScale < 1.0) Bitmap.createScaledBitmap(bmp, wW, wH, true) else bmp
        ...
        try {
            ...
            if (glarePct > c("GLARE_AREA_PCT_MAX")) {
                ...
                Imgproc.findContours(
                    thresh, contours, hierarchy, Imgproc.RETR_EXTERNAL,
    ...
    private fun calibrate(bgr: Mat, gray: Mat, opts: JSONObject,
        ...
        var pts: MatOfPoint2f? = null
        var objPts: MatOfPoint3f? = null
        var objImg: MatOfPoint2f? = null
        var HMat: Mat? = null
        var K: Mat? = null
        var ptsFull: MatOfPoint2f? = null
        var rvec: Mat? = null
        var tvec: Mat? = null
        var distCoeffs: MatOfDouble? = null
        var r: Mat? = null
        ...
        try {
            ...
            } else if (opts.has("marker_size_mm")) {
                markerSideMm = opts.optDouble("marker_size_mm")
            }
            ...
            objPts = MatOfPoint3f(
                ...
                Point3(half, half, 0.0), Point3(-half, half, 0.0))
            objImg = objToImg(objPts)
            HMat = Calib3d.findHomography(objImg, pts, 0)
            if (HMat == null || HMat.empty()) {
            ...
            ptsFull = MatOfPoint2f(*ptsFullArr.toTypedArray())

            rvec = Mat(); tvec = Mat(); distCoeffs = MatOfDouble()
            val ok = Calib3d.solvePnP(objPts, ptsFull, K, distCoeffs,
